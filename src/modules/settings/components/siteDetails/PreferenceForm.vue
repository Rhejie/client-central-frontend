			...
			<div class="md:col-span-1">
				<div class="px-4 sm:px-0 text-untitled-gray-600">
					...
					<p class="mt-1 text-sm text-untitled-gray-600">
						...
						elit. Voluptate nisi, quam vitae veniam odit iste velit
						suscipit!
					</p>
				</div>
			</div>
...
<script setup>
import { ref, defineProps, computed, defineAsyncComponent, defineEmits, watch } from 'vue';
import {
	MapPinIcon,
	PhotoIcon,
...
import { deletePreference } from '../../composables/siteDetails/siteDetails/delete_preference';
import { storePreference } from '../../composables/siteDetails/siteDetails/store_preference';
import { updatePreference } from '../../composables/siteDetails/siteDetails/update_preference';
...
const emit = defineEmits(['resetPreference', 'createdForm', 'updatedForm'])
...
const componentList = {
	preferenceInfo: defineAsyncComponent(() =>
		import('../../components/siteDetails/forms/PreferenceInfo.vue')
	),
	preferenceAddress: defineAsyncComponent(() =>
		import('../../components/siteDetails/forms/PreferenceAddress.vue')
	),
	preferenceLinks: defineAsyncComponent(() =>
		import('../../components/siteDetails/forms/PreferenceLinks.vue')
	),
	preferenceImage: defineAsyncComponent(() =>
		import('../../components/siteDetails/forms/PreferenceImage.vue')
	),
};
...
const editPreference = async () => {
	...
	errorMessage.value = message.value;
	loadingSaving.value = false;
	if(errorData.value && errorData.value.phone_number) {
		handleClickTag('Info')
	}
...
const initializeForm = () => {
	...
	formData.append('preferenceable_type', 'Client\\Company\\Models\\Company');
	formData.append(
		'preferenceable_id',
