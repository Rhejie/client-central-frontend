			...
			<MenuItems
				...
				<div class="py-1">
					...
					<MenuItem v-slot="{ active }">
						<a
							type="button"
							@click="handleClickEditUser(scope)"
							:class="[
								active
									? 'bg-gray-100 text-gray-900'
									: 'text-gray-700',
				...
				<div class="py-1">
					<MenuItem v-slot="{ active }">
						<a
							@click="handleClickDeleteUserConfirmation(scope)"
							:class="[
								active
									? 'bg-gray-100 text-gray-900'
									: 'text-gray-700',
...
const handleClickViewUser = (scope) => {
    router.push({ name: 'View User Page', params: { id: scope.id } })
}

const handleClickEditUser = (scope) => {
    router.push({ name: 'Edit User Page', params: { id: scope.id } })
}

const handleClickDeleteUserConfirmation = (scope) => {
...
const handleClickDeleteUser = async (scope) => {
	const { trash } = deleteUser(scope);
	await trash();
	notification(NotificationMessage.DELETED);
	handleCloseModal();
    setTimeout(() => {
        emit('deleteUser', scope)
    }, 500)
};
